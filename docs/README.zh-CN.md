...
面向多市场适配的证据优先研究工作流（含 A 股适配示例），重点增强两件事：专家团协作，以及有边界的自主进化。
...
- W1 生成 → W2 复盘 → W2.5 人工确认门 → W3 进化 → 下一轮 W1 的自主改进闭环

专家团机制见 [`专家团协作说明`](expert-team.md)，自主进化规则见 [`自主进化闭环`](evolution-loop.md)。

## 市场边界

公开核心不把 A 股制度当成所有市场的默认规则。交易日历、T+1（当天买入通常需下一个交易日才能卖出）结算、涨跌幅限制、做空规则、除权处理、汇率和流动性假设，都必须由具体市场适配器明确配置；示例不包含真实股票、持仓或账户数据。
